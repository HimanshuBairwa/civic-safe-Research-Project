...
by \emph{any} over-ID test at \emph{any} $K$, and show that negative-control
(proximal) channels restore identification when available, with honestly
untestable assumptions. On synthetic ground truth the over-ID test attains
...
exact ($0.90$) and latent coverage is $\approx 0.89$; proximal correction
cuts latent RMSE from $0.57$ to $0.28$ under a unit-strength common-mode
confounder and does no harm when none is present ($0.21$ vs.\ $0.21$). On
India NCRB state-year data (2001--2010, $N{=}253$, four institutional
channels) the one-factor law is not rejected in detectable directions
($p{=}0.088$) and the recovered latent ordering is face-valid. We make no
...
negative-control channels restore identification under explicit, \emph{untestable}
assumptions (Sec.~\ref{sec:impossible}).
\end{enumerate}
...
\paragraph{Point-identification and anytime-valid monitoring (implemented).}
...
$\mathrm{Var}(\theta)$ (Fig.~\ref{fig:core}, right; verified to recover the truth
to within $12\%$ across confounder strengths, with a detection gate that avoids
spurious correction when no common mode is present). A deployment monitor
(Fig.~\ref{fig:conf_monitor}, right) runs a Vovk--Wang $p$-to-$e$ calibrated
product e-process over the stream of per-period over-identification $p$-values;
by Ville's inequality it controls the \emph{anytime} false-alarm rate at
$\alpha$ (verified $0.013 \le 0.05$ over long null streams) while detecting a
structural drift with median delay $\approx 9$ windows.
...
channels ($p\approx 0.09$, no rejection). The test thus separates genuinely
...
side rather than presuppose the assumptions.
