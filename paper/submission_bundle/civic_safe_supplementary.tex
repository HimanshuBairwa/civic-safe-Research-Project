...
manuscript states without proof or only in brief. Section~\ref{sec:s1} gives the
...
cannot, and why. Four figures carry material that a table cannot: the shape of the
sensitivity frontier, the count distributions that decide what the ZINB head is
for, the in-degree distributions behind the dual-graph design, and the split of
predictive variance across ensemble seeds. Every value plotted is recomputed from
raw data or a persisted result file by
\texttt{scripts/make\_supplementary\_figures.py}, which also writes the plotted
numbers to \texttt{outputs/figure\_data/} so any panel can be checked against them.
Section~\ref{sec:s6} adds the tail score and the routing guarantee that consumes
it: threshold-weighted CRPS on the busiest decile, which is roughly twice the
aggregate, and finite-sample certificates on realized route exposure.
\end{abstract}
...
Notation follows Table~I of the main text exactly.
Section~\ref{sec:s1-scm} writes the loop as a causal graph and marks the one edge
the correction inverts, Section~\ref{sec:s1-jacobian}
...
\subsection{The loop as a causal graph}
\label{sec:s1-scm}

The main text writes the loop as a fixed point. Reading it as a causal graph
first is worth the half page, because it makes visible which single edge the
correction inverts and which quantities are observed.

Fix an area $s$ and a week $t$. Write $Z_s$ for the static covariates,
$\lambda_{s,t}$ for latent incidence, and $a_{s,t}$ for allocated attention. Write
$m_{s,t}$ for the recording multiplier, $y_{s,t}$ for the recorded count, and
$\mu_{s,t}$ for the learner's recorded-rate estimate. The structural assignments
are
\begin{align}
\lambda_{s,t} &\leftarrow f_{\lambda}(Z_s,\, \varepsilon_{s,t}),
\label{eq:s-scm-lambda}\\
a_{s,t} &\leftarrow \varphi(\mu_{s,t-1}),
\label{eq:s-scm-a}\\
m_{s,t} &\leftarrow g(a_{s,t}),
\label{eq:s-scm-m}\\
y_{s,t} &\leftarrow \mathrm{Poisson}\!\left(\lambda_{s,t}\, m_{s,t}\right),
\label{eq:s-scm-y}\\
\mu_{s,t} &\leftarrow \mathrm{learner}\!\left(y_{s,\le t},\, Z_s\right),
\label{eq:s-scm-mu}
\end{align}
which as a graph is the cycle
\begin{equation}
\label{eq:s-dag}
Z_s \rightarrow \lambda_{s,t} \rightarrow y_{s,t} \rightarrow \mu_{s,t}
\rightarrow a_{s,t+1} \rightarrow m_{s,t+1} \rightarrow y_{s,t+1}
\rightarrow \cdots
\end{equation}
with $m_{s,t}$ entering $y_{s,t}$ multiplicatively beside $\lambda_{s,t}$. Three
things follow, and the third is the contribution.

\emph{The cycle is not a violation of acyclicity.} Unrolled in time the graph is a
directed acyclic graph, one copy of \eqref{eq:s-scm-lambda}--\eqref{eq:s-scm-mu}
per week, with edges only forward in $t$. The fixed point of the main text is its
stationary solution, the configuration in which $\mu_{s,t} = \mu_{s,t-1}$ for
every $s$. So the fixed-point algebra and the causal reading describe one object.

\emph{Only $y$ and $Z$ are observed.} The latent rate $\lambda$, the attention
$a$, and the multiplier $m$ are all unobserved in our data. A forecaster trained
on records estimates the product $\lambda_{s,t} m_{s,t}$ and cannot separate the
factors from records alone, which is the identification problem the paper's
Section~V-E is honest about: $\kappa$ is not point-identified.

\emph{The correction is an intervention on one edge.} Deflation replaces the
recorded rate by $\hat{\lambda}_s = \mu_s / m_s$, which is exactly the operation
that deletes the $m_{s,t} \rightarrow y_{s,t}$ edge and leaves
$\lambda_{s,t} \rightarrow y_{s,t}$ intact. In do-calculus notation the target is
$\mathbb{E}[y_{s,t} \mid \mathrm{do}(m_{s,t} = 1)]$, the count that would have been
recorded had attention been uniform. Theorem~2(i) says this is computable in closed
form from $\mu$ and $\kappa$, with no unobserved constant left over. It also shows
why the estimand is a counterfactual and not an observed quantity: no week of real
data has $m \equiv 1$, so nothing in the record can validate the recovered value
directly. That is the formal reason latent coverage is checked in simulation only.
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.72\linewidth]{figS_gamma_frontier.pdf}
  \caption{The frontier of Table~\ref{tab:s-gamma}, drawn. Top: latent coverage of
  the plain corrected interval and of the $\Gamma$-inflated interval against the
  assumed misspecification factor, with the $0.90$ target and the shaded region in
  which the plain interval no longer meets it. Bottom: the width the inflated
  interval pays for that guarantee, relative to $\Gamma = 1$. The two quantities
  are drawn on separate panels sharing the $\Gamma$ axis rather than on twin
  vertical axes: they have different units, and a twin-axis rendering would let the
  apparent crossing point be placed anywhere by the choice of scaling. Generated by
  \texttt{scripts/make\_supplementary\_figures.py} from
  \texttt{outputs/gamma\_sensitivity\_frontier.json}.}
  \label{fig:s-gamma}
\end{figure}

Three readings, one of them unfavourable to us. Fig.~\ref{fig:s-gamma} shows all
three at once. The frontier is the honest object
...
\subsection{Count distributions and what the ZINB head is actually for}
\label{sec:s4-counts}

The main text fits a zero-inflated negative binomial likelihood. That is two
departures from a Poisson -- an atom at zero and a free dispersion -- and they are
not equally load-bearing. Fig.~\ref{fig:s-counts} separates them.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{figS_count_distributions.pdf}
  \caption{Chicago weekly counts per community area, 2018--2023, $77 \times 313$
  cell-weeks and $1{,}326{,}056$ incidents. Panels (a)--(c): the empirical
  distribution of each category against a Poisson null. The null is not a single
  Poisson at the pooled mean but a \emph{mixture of cell-specific Poissons}, each
  area at its own weekly mean -- the best a model can do knowing every area's level
  while allowing neither overdispersion nor a zero atom. Panel (d): observed
  zero-cell fraction against that null, log scale, with the ratio above each pair.
  Panel (e): the variance-to-mean ratio, which a Poisson fixes at $1$. Weekly bins
  are Monday-anchored, matching the sparsity audit reported in
  Section~\ref{sec:s5-categories}. Generated by
  \texttt{scripts/make\_supplementary\_figures.py} from
  \texttt{data/raw/chicago/*.parquet}; plotted values in
  \texttt{outputs/figure\_data/figS\_count\_distributions.json}.}
  \label{fig:s-counts}
\end{figure}

The zero atom buys less than the raw sparsity suggests. Drug crime is $50.89\%$
zero cells, which sounds like a decisive case for zero inflation. But areas differ
in level by an order of magnitude, and once the null is allowed to know each area's
own mean it already predicts $46.03\%$ zeros. The excess the atom has to explain is
a factor of $1.11$, not the factor of $4.37$ that a pooled-mean Poisson
($11.65\%$) would imply. Violent crime is a factor of $1.15$ over its null
($1.09\%$ against $0.95\%$) and property crime $1.74$ ($0.199\%$ against
$0.114\%$). We report the heterogeneous null rather than the pooled one because the
pooled comparison is a straw man: it attributes to zero inflation what is really
spatial variation in level, and it would overstate our own modelling choice by
about fourfold.

What the same figure shows unambiguously is overdispersion. The variance-to-mean
ratio is $19.06$ for violent, $27.86$ for property and $13.75$ for drug crime,
against $1$ for any Poisson. Panels (a)--(c) show where that lives: the empirical
right tail runs far past the null in every category, most visibly on drug crime,
where observed cell-weeks reach $68$ while the null has effectively no mass beyond
about $35$. So the negative binomial component is doing the heavy lifting and the
zero atom is a modest refinement on top of it. That ordering is the honest reading,
and it is the opposite of the emphasis the phrase ``zero-inflated'' invites. The
atom still earns inclusion -- Chicago's Brier score on the zero event is $0.0592$,
so it is measurably active -- but a reader deciding whether to adopt ZINB over a
plain negative binomial should expect a small gain, not the large one that
$50.89\%$ zeros seems to promise.
...
Fig.~\ref{fig:s-graphs} plots the full in-degree distributions behind those
summary statistics, and the shapes carry information the table's five columns
cannot. Queen contiguity is tightly concentrated -- Chicago piles up at degrees
four to six, New York at one to four -- because polygon adjacency in a partition of
a city is bounded by geometry. The $k$-NN in-degree spreads much wider, from $1$ to
$13$, because being a near neighbour is not a symmetric relation: a unit at the
centre of a dense cluster is chosen by many others, one at the edge of the city by
few. The two distributions barely overlap, which is the clearest statement of why
the second graph is not a noisy copy of the first. And New York's isolated
precinct, invisible in a mean and reported as a single $0$ in the table, is the
leftmost bar of the right-hand panel: one unit that queen contiguity leaves with no
neighbours at all, and which the $k$-NN graph restores to a degree of at least two.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{figS_graph_degree.pdf}
  \caption{In-degree distributions for the two adjacency structures, computed from
  the released boundary files by \texttt{build\_adjacency\_from\_geodataframe} with
  $k = 8$ and the projected CRS named in the text. Queen contiguity is bounded by
  polygon geometry and concentrates; centroid $k$-NN spreads because the
  nearest-neighbour relation is asymmetric. The single New York precinct at
  in-degree $0$ under queen contiguity, marked, is the concrete defect the dual
  graph exists to repair. Generated by
  \texttt{scripts/make\_supplementary\_figures.py}; plotted values in
  \texttt{outputs/figure\_data/figS\_graph\_degree.json}.}
  \label{fig:s-graphs}
\end{figure}
...
therefore the one with the least room to beat a trailing mean. Note that this is
not the same as saying zero inflation is doing the most work there:
Section~\ref{sec:s4-counts} shows the atom accounts for a factor of only $1.11$
over a null that already knows each area's own level, and that the dominant
departure from Poisson in every category is overdispersion. What makes drug crime
hard is that a mean of $2.15$ per cell-week leaves little signal above the noise,
not that the likelihood is straining. The aggregate
...
\subsection{Where the predictive variance comes from}
\label{sec:s5-uncertainty}

The five-seed ensemble makes it possible to split predictive variance into an
aleatoric part, the spread the ZINB head assigns within a seed, and an epistemic
part, the disagreement between seeds. Fig.~\ref{fig:s-uncertainty}(a) gives the
split. Epistemic variance is $2.6\%$ of the total in Chicago and $4.3\%$ in New
York, on totals of $33.77$ and $59.96$ squared counts. Almost all the uncertainty
we report is irreducible noise in the recording process rather than ignorance about
parameters, which is the expected ordering for count data at this spatial
resolution and this much history, and it bounds what more seeds could buy.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{figS_ensemble_uncertainty.pdf}
  \caption{(a) Aleatoric and epistemic components of predictive variance, both
  cities, with the epistemic share annotated. (b) Test CRPS of the five ensemble
  members against the CRPS of their EMOS combination. The combination beats the
  \emph{mean} member in both cities, but in Chicago one member reaches $2.8182$ and
  so beats the combination itself; that member is identifiable only in hindsight,
  which is why the ensemble remains the right choice and why we show the point
  rather than the average alone. These five seeds are the members of the reported
  ensemble; the separate single-model comparison against the deep baselines in
  Section~\ref{sec:s3-protocol} uses the seed-matched protocol and a different
  number. Generated by \texttt{scripts/make\_supplementary\_figures.py} from
  \texttt{outputs/conformal\_evaluation/*\_conformal\_results.json}.}
  \label{fig:s-uncertainty}
\end{figure}

Panel (b) is the part we would rather a referee saw from us. The EMOS combination
improves on the mean member in both cities, $2.8730$ to $2.8267$ in Chicago and
$3.2033$ to $3.1401$ in New York. It does not dominate every member: Chicago's best
single seed scores $2.8182$, which is better than the ensemble's $2.8267$. Picking
that seed requires knowing the test score first, so the comparison is not one a
deployment could win, and averaging remains the defensible choice. But it does mean
the ensemble's contribution in Chicago is to remove seed variance rather than to
reach a level no seed reaches, and those are different claims. In New York the
ensemble does beat all five members, its best being $3.1609$.

One further detail belongs here because it is easy to miss in the results files.
The category-conditioned EMOS weighting fell back to uniform weighting in both
cities on the internal calibration holdout. In New York the learned weights are
marginally worse on test than equal weights, $3.1401$ against $3.1400$, so the
learned combination earns essentially nothing there and we report the learned
number rather than selecting the better of the two after the fact.
...
% =====================================================================
\section{Tail Calibration and Route Exposure Certificates}
\label{sec:s6}
% =====================================================================

The main text reports aggregate CRPS and marginal coverage. Neither says how the
model behaves on the weeks that matter operationally, which are the busy ones. This
section reports the tail score we compute and the routing guarantee that consumes
it, and it says plainly where each one stops.

\subsection{Threshold-weighted CRPS on the upper tail}
\label{sec:s6-tail}

Aggregate CRPS averages over all cell-weeks, so a model can score well by being
sharp on quiet cells while missing surges. The threshold-weighted CRPS of
Gneiting and Ranjan reweights the integrand to a region of interest. We evaluate
it on the empirical upper decile, taking the threshold as the $90$th percentile of
observed counts. Table~\ref{tab:s-tail} reports the result for the calibrator
selected in each city.

\begin{table}[!t]
  \centering
  \caption{Threshold-weighted CRPS on the upper decile of observed counts. The
  threshold is the empirical $90$th percentile; \texttt{tail\_n} is the number of
  cell-weeks at or above it. ``twCRPS tail'' is the score restricted to those
  cells, and the final column is its ratio to aggregate CRPS. Read from
  \texttt{outputs/tail\_metrics/*.json}.}
  \label{tab:s-tail}
  \begin{tabular}{l r r r r r r}
    \toprule
    \textbf{City} & \textbf{Thresh.} & \textbf{tail $n$}
    & \textbf{tail frac.} & \textbf{twCRPS} & \textbf{twCRPS tail}
    & \textbf{ratio to CRPS} \\
    \midrule
    Chicago  & 51 & 1291 & 0.1054 & 0.7444 & 6.3787 & $2.26\times$ \\
    New York & 61 & 1278 & 0.1030 & 0.7677 & 6.4582 & $2.06\times$ \\
    \bottomrule
  \end{tabular}
\end{table}

The number to take from the table is the ratio, and it is unflattering. On the
busiest decile the score is $2.26\times$ the aggregate in Chicago and
$2.06\times$ in New York. Forecasting is materially harder exactly where a
deployment would care most. The low overall twCRPS ($0.7444$, $0.7677$) is not
evidence against this. The weight function is near zero on
the $89\%$ of cells below threshold. So the aggregate twCRPS is small by
construction, and only the tail-restricted column belongs beside CRPS. We report
both so the comparison cannot be made the wrong way round.

\subsection{Conformal route exposure certificates}
\label{sec:s6-exposure}

A civilian following an advisory route cares about the risk they actually meet,
not the risk the model predicted. Those differ whenever the predicted field is
wrong. Fix a routing policy $\pi$ mapping a predicted node-risk field to an
ordered path. On $n$ calibration scenarios, each carrying a predicted field and
the risk field that materialized, score the realized exposure
\begin{equation}
\label{eq:s-exposure}
E_i \;=\; \sum_{s \,\in\, \pi(\text{predicted}_i)} \text{realized}_i[s],
\end{equation}
and let $Q$ be the $k$th smallest of $\{E_i\}$ with
$k = \lceil (n+1)(1-\alpha) \rceil$. Split conformal then gives
\begin{equation}
\label{eq:s-exposure-guarantee}
\Pr\!\left(E_{n+1} \le Q\right) \;\ge\; 1 - \alpha
\end{equation}
for an exchangeable new scenario. Two features of \eqref{eq:s-exposure-guarantee}
are worth stating. It needs no monotonicity in the router's risk aversion. That
matters because realized exposure is \emph{not} monotone in aversion: planning on
a mis-calibrated field can route a civilian through a genuinely dangerous node. A
conformal-risk-control argument over a monotone threshold family would therefore be
unsound here. And it is a bound on a scalar functional of the route, not on each
node's risk, which is what makes it distribution-free at this strength.

Table~\ref{tab:s-exposure} measures whether the bound holds in practice, and
compares two policies: one planning on the recorded field, one on the deflated
field $\hat{\lambda}$ of Theorem~2. Reproduced by
\texttt{scripts/exposure\_certificate\_study.py}; raw values in
\texttt{outputs/routing/exposure\_certificates.json}.

\begin{table}[!t]
  \centering
  \caption{Empirical breach rate of the certified exposure bound,
  $400$ trials per row, $n = 150$ calibration scenarios, $40$ nodes, routes of
  $8$ nodes, feedback gain $\kappa = 0.6$. ``Breach'' is the fraction of trials
  whose realized exposure exceeded the certificate; the guarantee caps it at
  $\alpha$. Bound and exposure columns are trial means.}
  \label{tab:s-exposure}
  \begin{tabular}{l r r r r l}
    \toprule
    \textbf{Plans on} & $\alpha$ & \textbf{breach}
    & \textbf{cert. bound} & \textbf{realized} & \textbf{within $\alpha$?} \\
    \midrule
    record   & 0.05 & 0.0475 & 16.31 & 9.66 & yes \\
    record   & 0.10 & 0.0775 & 14.31 & 9.19 & yes \\
    record   & 0.20 & 0.1725 & 12.40 & 9.42 & yes \\
    \addlinespace
    deflated & 0.05 & 0.0225 & 16.19 & 9.23 & yes \\
    deflated & 0.10 & 0.1100 & 14.34 & 9.96 & no (see text) \\
    deflated & 0.20 & 0.1850 & 12.42 & 9.50 & yes \\
    \bottomrule
  \end{tabular}
\end{table}

Five of the six rows sit at or below their target. One does not: the deflated
policy at $\alpha = 0.10$ breaches on $11.00\%$ of trials against a $10\%$ cap. We
report it rather than enlarging the trial count until it disappears. With $400$
trials the standard error at $\alpha = 0.10$ is $0.015$, so $0.1100$ is
$0.67$ standard errors above target, and a one-sided binomial test of
$H_0: \Pr(\text{breach}) \le 0.10$ gives $p = 0.275$. The row is consistent with
the guarantee holding and with finite-sample noise; it is not evidence of a
violation. A reader who wants the cleaner table can raise the trial count, and the
script takes it as a constant.

Note also what the certificate does \emph{not} deliver. It bounds realized
exposure for the policy it was calibrated on. A policy chosen by looking at these
numbers therefore needs a fresh fold; \texttt{select\_risk\_budget} in the same
module implements that split, and we report the certificate rather than a selected
policy. The certified bounds also sit well above mean realized exposure ($16.31$
against $9.66$ at $\alpha = 0.05$). That gap is the usual price of a
distribution-free upper quantile, not a sharpness claim.

The risk fields in Table~\ref{tab:s-exposure} are simulated, and that is a
limitation of the same kind as the latent-coverage one. The realized field in the
crime setting is latent incidence, which real records never reveal, so a study
requiring it cannot be run on Chicago or New York. What Table~\ref{tab:s-exposure}
establishes is that the certificate machinery behaves as its proof says under a
feedback-inflated recording process. It is a mechanism check, not field evidence,
and the routing layer stays advisory throughout the paper for this reason.
